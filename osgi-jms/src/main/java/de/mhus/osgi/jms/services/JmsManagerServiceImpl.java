 ...
 *
 * <p>Discussion: https://github.com/eclipse/smarthome/issues/870
 *
 * <p>Switching to DS driven references was also not successful, got the same exception.
 *
 * <p>You can chnage the behavior in mhus-config by setting JmsManagerService/startupDelay to zero
 *
 * @author mikehummel
 */
...
public class JmsManagerServiceImpl extends MLog implements JmsManagerService {
    ...
    public void doStart(ComponentContext ctx) {
        ...
        connectionTracker =
                new ServiceTracker<>(
                        context, JmsDataSource.class, new MyConnectionTrackerCustomizer());
        connectionTracker.open();

        channelTracker =
                new ServiceTracker<>(
                        context, JmsDataChannel.class, new MyChannelTrackerCustomizer());
        channelTracker.open();
    }
    ...
    public void doDeactivate(ComponentContext ctx) {
        ...
        for (String name : new LinkedList<String>(channels.keySet())) removeChannel(name);
    ...
    public JmsConnection getConnection(String name) {
        ...
        try {
            JmsDataSource src =
                    MOsgi.getService(
                            JmsDataSource.class, "(osgi.jndi.service.name=jms_" + name + ")");
    ...
    private class MyConnectionTrackerCustomizer
        ...
        public void modifiedService(
                ServiceReference<JmsDataSource> reference, JmsDataSource service) {
    ...
    private class MyChannelTrackerCustomizer
        ...
        @Override
        public void modifiedService(
                ServiceReference<JmsDataChannel> reference, JmsDataChannel service) {
    ...
    public JmsDataChannel getChannel(String name) {
        try {
            return MOsgi.getService(
                    JmsDataChannel.class, "(osgi.jndi.service.name=jmschannel_" + name + ")");
        } catch (NotFoundException nfe) {
            for (JmsDataChannel c : getChannels()) if (name.equals(c.getName())) return c;
    ...
    public String getServiceName(Service<JmsDataSource> ref) {
        ...
        if (p != null
                && p instanceof String
                && ((String) p).length() > 4
                && ((String) p).startsWith("jms_")) return ((String) p).substring(4);
        return null;
    }
