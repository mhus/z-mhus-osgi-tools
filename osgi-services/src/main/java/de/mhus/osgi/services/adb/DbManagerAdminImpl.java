...
@Component(
        service = DbManagerAdmin.class,
        immediate = true,
        name = "de.mhus.lib.karaf.adb.DbManagerAdmin")
public class DbManagerAdminImpl extends MJmx implements DbManagerAdmin {
    ...
    private class MyTrackerCustomizer
        ...
        public void modifiedService(
                ServiceReference<DbManagerService> reference, DbManagerService service) {}
    ...
    public DbManagerService getService(String name) {
        ...
        synchronized (services) {
            for (DbManagerService service : services) {
                if (name.equals(service.getServiceName())) return service;
            }
        }
        return null;
    }
