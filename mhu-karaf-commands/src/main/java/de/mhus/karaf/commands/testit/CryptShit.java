...
//			System.out.println("KeyLength: " + key.length + " (" + (key.length*8) + " bytes)");
			byte[] enc = MBouncy.encryptAes(key, parameters[1].getBytes("utf-8"));
			System.out.println("Real     : " + Base64.encode(parameters[1].getBytes("utf-8")));
		...
		case "blowfish.type.encrypt": {
	        ...
	        SecretKeySpec skeyspec=new SecretKeySpec(parameters[2].getBytes("utf-8"),parameters[0]);
	        Cipher cipher= provider.length() == 0 ? Cipher.getInstance(parameters[0]) : Cipher.getInstance(parameters[0],provider);
		...
		case "hmacsha1": { // RFC2104HMAC
			SecretKeySpec signingKey = new SecretKeySpec(parameters[0].getBytes(), "HmacSHA1");
			Mac mac = Mac.getInstance("HmacSHA1");
			mac.init(signingKey);
			System.out.println(MCast.toHexString(mac.doFinal(parameters[1].getBytes())));
		} break;
		case "aes.encrypt": {
			System.out.println(encrpytString(parameters[0], parameters[1], parameters[2])); // not working correct ...
		} break;
		case "aes.decrypt": {
			System.out.println(decrpytString(parameters[0], parameters[1], parameters[2])); // not working correct ...
		} break;
		default:
