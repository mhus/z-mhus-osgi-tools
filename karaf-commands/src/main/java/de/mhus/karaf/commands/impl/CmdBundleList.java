...
public class CmdBundleList extends AbstractCmd {
    ...
    @Option(
            name = "-m",
            aliases = {"--modified"},
            description = "Order by modify date",
            required = false,
            multiValued = false)
    ...
    @Option(
            name = "-s",
            aliases = {"--symbolic"},
            description = "Order by symbolic name",
            required = false,
            multiValued = false)
    ...
    public Object execute2() throws Exception {
        ...
        if (pLocation) table.addHeader("Location");
            ...
            Collections.sort(
                    ...
                    new Comparator<Object[]>() {
                        ...
                        public int compare(Object[] o1, Object[] o2) {
                            int ret =
                                    Long.compare(
                                            ((Bundle) o1[0]).getLastModified(),
                                            ((Bundle) o2[0]).getLastModified());
                            if (ret == 0)
                                ret =
                                        ((Bundle) o1[0])
                                                .getSymbolicName()
                                                .compareTo(((Bundle) o2[0]).getSymbolicName());
                            return ret;
                        }
                    });
        } else if (orderSymbolic) {
            Collections.sort(
                    list,
                    new Comparator<Object[]>() {
        ...
        for (Object[] l : list) {
            ...
            if (pLocation)
                table.addRowValues(
                        ...
                        toState(b.getState()),
                        MDate.toIsoDateTime(b.getLastModified()),
                        b.getLocation(),
                        valid);
            else
                table.addRowValues(
