...
public class CmdIdent extends AbstractCmd {
    ...
    @Option(
            name = "-a",
            aliases = "--all",
            description = "Print also hostname and pid",
            required = false)
    ...
    @Option(
            name = "-p",
            aliases = "--print",
            description = "Print also attributes",
            required = false)
    ...
    public Object execute2() throws Exception {
        ...
        if (attr) System.out.println(ServerIdent.getAttributes());
        return service.toString()
                + (full ? " " + MSystem.getPid() + "@" + MSystem.getHostname() : "");
    }
}
