...
public class CmdRun extends AbstractCmd {
    ...
    @Option(
            name = "-i",
            aliases = {"--notinteruptable"},
            description =
                    "Not interuptable by pressing Ctrl+C, this will allow the script to process binary stdin",
            required = false,
            multiValued = false)
    ...
    @Option(
            name = "-c",
            aliases = {"--command"},
            description = "direct command input",
            required = false,
            multiValued = false)
    boolean command;
    ...
    @Option(
            name = "-g",
            aliases = {"--goto"},
            description = "First goto jump to start",
            required = false,
            multiValued = false)
    String go;
    ...
    @Option(
            name = "-s",
            aliases = {"--sensitive"},
            description = "do not separate commands by semikolon",
            required = false,
            multiValued = false)
    boolean sensitive;
    ...
    public Object execute2() throws Exception {
        ...
        if (command) {
            ...
            if (!sensitive) fileName = fileName.replace(";", "\n");
    ...
    private class Context {
        ...
        private boolean findEndIf(Session session) throws Exception {
            ...
            while (true) {
                ...
                else if (line.equals("else")) ifCnt--;
                else if (line.startsWith("elseif ")) {
                    if (ifCnt == 0) {
                        if (session == null) {
                        } else if (isTrue(session.execute("%(" + line.substring(7) + ")"))) {
                            return true;
                        }
                    }
                } else if (line.equals("endif")) ifCnt--;
        ...
        private boolean findDone() throws Exception {
            ...
            while (true) {
                ...
                else if (line.startsWith("while ")) ifCnt++;
                else if (line.equals("done")) ifCnt--;
