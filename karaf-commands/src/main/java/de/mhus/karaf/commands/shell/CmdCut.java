...
public class CmdCut extends AbstractCmd {
    ...
    @Option(
            name = "-d",
            aliases = {"--delimiter"},
            description = "Seperate parts by this regex",
            required = false,
            multiValued = false)
    String delim;
    ...
    @Option(
            name = "-j",
            aliases = {"--join"},
            description = "Glue between fields",
            required = false,
            multiValued = false)
    String join = "";
    ...
    @Option(
            name = "-t",
            aliases = {"--trim"},
            description = "Trim every single part",
            required = false,
            multiValued = false)
    boolean trim = false;
    ...
    @Option(
            name = "-n",
            description = "Add not new line at the end",
            required = false,
            multiValued = false)
    boolean n = false;
    ...
    @Option(
            name = "-x",
            description = "Do not ignore empty lines",
            required = false,
            multiValued = false)
    boolean empty = false;
    ...
    @Option(
            name = "-m",
            aliases = {"--mode"},
            description = "How to handle the value, text, array -f , list -p",
            required = false,
            multiValued = false)
    String mode;
    ...
    public Object execute2() throws Exception {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (mode == null || mode.equals("text")) {
                ...
                if (MString.isSet(delim)) processDelim(line);
                else if (MString.isSet(positions)) processPos(line);
                else {
                    out.append(line);
                    if (!n) out.append("\n");
                }
                ...
                if (line.startsWith("[") && line.endsWith("]")) {
                    ...
                    if (fields != null) {
                        for (String f : fields.split(",")) {
                            ...
                            if (map.containsKey(f)) out.append(map.get(f));
                        }
                    }
                    if (!n) out.append("\n");
                }
            ...
            if (empty || out.length() > 0) System.out.print(out);
        }
        //		return out.toString();
        return null;
    }
