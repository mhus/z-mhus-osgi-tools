...
@Command(
        ...
        name = "reinstall",
        description =
                "Remove from local repository and reinstall bundles from the remote repository")
@Service
public class CmdBundleReinstall extends AbstractCmd {
    ...
    @Argument(
            ...
            name = "bundle",
            required = true,
            description = "(Raw) Name of the bundle to redeploy, use placeholder asterisk",
            multiValued = true)
    String[] bundleName;
    ...
    @Option(
            name = "-p",
            aliases = {"--pom"},
            description = "Read POM File Instead Of Location Information",
            required = false,
            multiValued = false)
    boolean usePom;
    ...
    @Option(
            name = "-n",
            aliases = {"--noupdate"},
            description = "Do not update automatically",
            required = false,
            multiValued = false)
    boolean notUpdate;
    ...
    @Option(
            name = "-v",
            aliases = {"--verbose"},
            description = "Print errors and verbose information",
            required = false,
            multiValued = false)
    ...
    @Option(
            name = "-w",
            aliases = {"--watch"},
            description = "Set bundle:watch option",
            required = false,
            multiValued = false)
    boolean setWatch;
    ...
    @Option(
            name = "-c",
            aliases = {"--clean"},
            description = "Clean Local Maven Repository",
            required = false,
            multiValued = false)
    ...
    public Object execute2() throws Exception {
        ...
        for (Bundle b : context.getBundles())
            for (String bn : bundleName) {
                if (MString.compareFsLikePattern(b.getSymbolicName(), bn)
                        || String.valueOf(b.getBundleId()).equals(bn)) {
                    try {
                        doIt(b);
                    } catch (Throwable t) {
                        System.out.println("ERROR: Bundle: " + bn + " " + t.toString());
                        if (printVerbose) t.printStackTrace();
                    }
                    break;
                }
            }
    ...
    public void doIt(Bundle bundle) throws Exception {
        ...
        if (printVerbose) System.out.println("Reinstall " + bn);
        ...
        if (usePom) {
            ...
            if (version.endsWith(".SNAPSHOT"))
                version = version.substring(0, version.length() - 9) + "-SNAPSHOT"; // hack!
        ...
        if (printVerbose)
            System.out.println(
                    "Redeploy: " + groupId + ":" + artifactId + ":" + version + " " + path);
        if (!file.exists()) {
            System.out.println("Local repository directory not found: " + bn);
        } else {
            if (cleanLocalRepo) MFile.deleteDir(file);
        }
        ...
        if (setWatch) session.execute("bundle:watch " + bundle.getSymbolicName());
    }
