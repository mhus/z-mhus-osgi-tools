...
public class CmdNewInstance extends AbstractCmd {
    ...
    @Option(
            name = "-p",
            aliases = {"--proxy"},
            description = "Creates an proxy and call the shell command",
            required = false,
            multiValued = false)
    String proxyCmd;
    ...
    public Object execute2() throws Exception {
        ...
        Class<?> clazz = null;
        if (className instanceof Class) clazz = (Class<?>) className;
        else if (className instanceof String) clazz = cl.loadClass(String.valueOf(className));
        else clazz = className.getClass();
        ...
        if (proxyCmd != null) {
            Object obj =
                    Proxy.newProxyInstance(
                            ...
                            new Class<?>[] {clazz},
                            new MyInvocationHandler(session, clazz, proxyCmd));
            if (set != null) session.put(set, obj);
            return obj;
        }
        ...
        if (pt != null) {
            ...
            for (int i = 0; i < p.length; i++) {
                if (p[i].equals("?")) parameterTypes[i] = parameters[i].getClass();
                else {
                    ...
                    parameters[i] =
                            MCast.toType(
                                    parameters[i],
                                    parameterTypes[i],
                                    MCast.getDefaultPrimitive(parameterTypes[i]));
                }
            }
    ...
    private static class MyInvocationHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            if (method.getName().equals("toString")) return "Proxy of " + clazz.getCanonicalName();
