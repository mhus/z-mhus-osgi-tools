...
@Command(
        scope = "jdbc",
        name = "blueprint",
        description = "Print blueprint of the datasource or a list of blueprints")
@Service
        ...
        if (source == null) {
            ...
            for (File f : deployFolder.listFiles()) {
                if (f.isFile() && f.getName().startsWith("datasource-")) {
                    System.out.println(
                            MString.beforeLastIndex(MString.afterIndex(f.getName(), '-'), '.'));
                }
            }
            ...
            if (!datasourceFile.exists()) {
                throw new IllegalArgumentException(
                        "The JDBC datasource file " + datasourceFile.getPath() + " doesn't exist");
            }
