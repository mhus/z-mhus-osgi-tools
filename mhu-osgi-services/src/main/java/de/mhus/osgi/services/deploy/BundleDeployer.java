...
public class BundleDeployer extends MLog {
	...
	public static File deploy(Bundle bundle, String bundlePath, SENSIVITY sensivity) throws MException {
	...
	public BundleDeployer(Bundle bundle, String bundlePath, File target, SENSIVITY sensivity) throws MException {
		setTarget(target);
	...
	private void doExport(Bundle bundle, URL entry) {
		...
		bundleDirectory = new File(target, config.getString("name", bundle.getSymbolicName() ) + "/" + bundlePath);
		
		if (sensivity.ordinal() <= SENSIVITY.CHECK.ordinal() || bundleDirectory.exists() && sensivity.ordinal() <= SENSIVITY.WRITE.ordinal() ) return;
		...
		while (list.hasMoreElements()) {
			...
			if (!path2.endsWith("/" + DEPLOY_PROPERTIES))
				doExport(bundle,path2, path.length()-1, bundleDirectory, config, sensivity.ordinal() <= SENSIVITY.UPDATE.ordinal());
		}
...
//				if (f.exists() && update && ) return; // Update is not working, have no modify date of the origin
	...
	public void setTarget(File target) throws MException {
		...
		if (!target.isDirectory())
			throw new MException("target must be a directory",target);
		this.target = target;
	}
	...
	public void setBundlePath(String bundlePath) {
		...
		if (!this.bundlePath.endsWith("/")) 
			this.bundlePath = this.bundlePath + "/";
	}
